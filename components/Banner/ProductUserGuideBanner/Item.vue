<template>
  <NuxtLink v-show='!manual.hidden' class='shadow-xl bg-white object-contain manual-slide max-w-[24.5rem] min-w-[21rem] mr-12 last:mr-0 rounded-2xl p-4 flex flex-col justify-between' :to='manualUrl'>
    ...
    <div class='mobile-slider-bottom flex items-center'>
      ...
      <span class='pl-4 uppercase manual-slide-title text-18 md:text-20 font-semibold'>{{ manual.title }}</span>
    </div>
  </NuxtLink>
</template>
<script>
...
export default {
  name: 'ProductUserGuideBannerListItem',
  props: {
    manual: {
      type: Object,
      required: true
